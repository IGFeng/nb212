...
<div id="main">
    <div id="submit">
    <form name="form1" method="post" action="{{url('thedit',['id'=>$content->id,'nickname'=>$content->nickname,'account'=>$content->account])}}">
        @csrf
    <div style="font-weight: 600;color:#4f98ca;" id="reply_nol">
    {{$content->username}}<span style="color:#999;font-weight:normal">{{$content->nickname}}于{{$content->systime}}发表留言：</span>
    </div>
